...
import { ArrowLeftIcon, Briefcase, ChevronLeft, ChevronRight, DownloadIcon, EyeIcon, EyeOffIcon, FileTextIcon, FolderIcon, GraduationCap, Share2Icon, Sparkle, Sparkles, User} from 'lucide-react'
...
const ResumeBuilder = () => {
  ...
  const changeVisibility = async () => {
    try {
      ...
      toast.success("Resume visibility updated successfully!")
    } catch (error) {
      console.error("Failed to update resume visibility.")
    }
  }

  const shareResume = () => {
    const resumeLink = window.location.href.split('/app')[0];
    const resumeUrl = resumeLink + '/view/' + resumeId;

    if(navigator.share){
      navigator.share({url: resumeUrl, title: resumeData.title,})
    }else{
      alert("Sharing is not supported in this browser. Copy the link: " + resumeUrl)
    }
  }
  ...
  const saveResume = async () => {
    try {
      ...
      //remove image file from personal info before sending to backend
        ...
        <div className='grid lg:grid-cols-12 gap-8'>
          ...
          <div className='relative lg:col-span-5 rounded-lg overflow-hidden'>
            <div className='bg-white rounded-lg shadow-sm border border-gray-300 p-6 pt-1'>
              ...
              <hr className='absolute top-0 left-0 h-1 bg-gradient-to-r from-blue-400 to-blue-600 border-none transition-all duration-2000'
              ...
              <div className='flex justify-between items-center mb-6 border-b border-gray-300 py-1'>
                ...
                <div className='flex items-center'>
                  ...
                  <button onClick={()=>setActiveSectionIndex((prevIndex)=>Math.min(prevIndex +1 , sections.length -1))} className={`flex items-center gap-1 p-3 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 transition-all ${activeSectionIndex === sections.length -1 ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={activeSectionIndex === sections.length -1}>Next <ChevronRight className='size-4 '/></button>
              ...
              <div className='space-y-6'>
                {activeSection.id === 'personal' && (
                  <PersonalInfoForm data={resumeData.personal_info} onChange={(data)=>setResumeData(prev => ({...prev, personal_info: data}))} removeBackground={removeSection} setRemoveBackground={setRemoveSection} />
                )}
                {
              ...
              <button onClick={()=>{toast.promise(saveResume, {loading: 'Saving...'})}} className='bg-gradient-to-br from-indigo-100 to-indigo-200 ring-indigo-300 text-blue-700 ring hover:ring-blue-400 transition-all rounded-md px-6 py-2
              mt-6 text-sm'>
                Save Changes
              </button>
            </div>
